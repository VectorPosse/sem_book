...
These six rules have important justifications. Don't just memorize the rules blindly. Understand why they are imperative.
...
Let's revisit the first model (of the four shown earlier):
...
* We need to name our variables. When we model real-world data, we'll use contextually meaningful names, but for abstract models we draw, we need a consistent way of labeling them.
    ...
    * Error terms will be called $E_{i}$ with its subscript matching the one on the endogenous variable to which it's attached.

* We need to label the parameters along the various paths of the model:
    * Variances will be called $v_{i}$ and the subscript will match the exogenous variables $X_{i}$ to which they're attached.
    * Error variances will be called $e_{i}$ and the subscript will match the error terms $E_{i}$ to which they're attached.
...
Let's now include all the extra bits of the model required by the aforementioned rules: a variance term for the exogenous variable $X_{1}$ and error terms for the two endogenous variables $Y_{1}$ and $Y_{2}$ along with labels for everything.
