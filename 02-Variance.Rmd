...
Suppose that $C$ is a constant, meaning a single number (rather than a variable that could represent lots of different numbers). Then,
...
Variances are not really interpretable directly. How do we make them more interpretable? Well, if variance has "squared" unit, we can take the square root to get back to the natural units we started with.
...
For SEM, we will focus almost exclusively on variance and switch to standard deviation only if we need to communicate something about spread in meaningful units.
